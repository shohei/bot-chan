...
fin = codecs.open(str(os.path.dirname(__file__))+"/trimmed_bocchan.txt","r","utf-8")
fout = codecs.open(str(os.path.dirname(__file__))+"/out_markov.txt","w","utf-8")
filearray = []
for line in fin:
    filearray.append(line)
fin_2gram = codecs.open(str(os.path.dirname(__file__))+"/2gram.txt","r","utf-8")
bigram_array = []
...
def index(request):
    if request.POST:
        conversation_request = request.POST['conversation']

        sent = conversation(conversation_request)
        sent_answer=""
        for s in sent:                                            
            if(type(s)==type('あ')): #str                                    
                sent_answer += s
                #print s,                                                    
            else: #unicode                                                    
                sent_answer += s.encode('utf-8')
                #print s.encode('utf-8'),                                    
        #print s,                                                            
        #print         

        return render_to_response('bocchan/index.html',{'sent':sent_answer,'conversation':conversation_request},context_instance=RequestContext(request))
    else:
        return render_to_response('bocchan/index.html')
